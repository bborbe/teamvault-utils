...
|---------|--------------|-------------|------------|
| **Binary + library** | git tag `vX.Y.Z` + matching `## vX.Y.Z` section in `CHANGELOG.md` | `go install github.com/Seibert-Data/teamvault-cli/v5@latest`; downstream Go projects importing the library | Auto-tagged by the project's own dark-factory daemon (`autoRelease: true`) when a prompt completes and updates `## Unreleased` |
...
The git tag is sufficient for `go install github.com/Seibert-Data/teamvault-cli/v5@vX.Y.Z`. A GitHub Release object is separate (see next section).
...
teamvault-cli --help 2>&1 | head -1  # sanity check

# Downstream / fresh machine:
go install github.com/Seibert-Data/teamvault-cli/v5@vX.Y.Z
