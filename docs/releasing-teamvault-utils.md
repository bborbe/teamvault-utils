...
Unlike `vault-cli` / `dark-factory` / `coding` which ship both a binary and a Claude Code plugin, `teamvault-utils` is **binary-only**: a Go library + seven CLI binaries (`teamvault-login`, `teamvault-password`, `teamvault-username`, `teamvault-url`, `teamvault-file`, `teamvault-config-parser`, `teamvault-config-dir-generator`) distributed via Go modules.

| Surface | Versioned by | Consumed by | Bumped how |
|---------|--------------|-------------|------------|
| **Binary + library** | git tag `vX.Y.Z` + matching `## vX.Y.Z` section in `CHANGELOG.md` | `go install github.com/bborbe/teamvault-utils/v4/cmd/teamvault-*@latest`; downstream Go projects importing the library | Auto-tagged by the project's own dark-factory daemon (`autoRelease: true`) when a prompt completes and updates `## Unreleased` |
...
Both bugs were caught only when a scenario was walked against a built binary against the real macOS Keychain. The rule: **before approving any dark-factory prompt that touches `keychain*.go`, `factory/`, or `cmd/`, walk the relevant active scenarios against a freshly built binary.** Surface-scoped skipping is acceptable when the diff is genuinely empty (see below).
...
```bash
# 1. Build a fresh binary set (NOT the installed ones)
...
|---------|---------------------|
| `teamvault-login` returns "inappropriate ioctl for device" when piped | `cmd/teamvault-login/main.go` `termReader.Read` — `term.ReadPassword(stdin.Fd())` requires a real tty; the bufio prompt path is not reachable from piped stdin. The pre-existing behavior is "keychain has a password OR user types at tty" |
...
# empty output → installed binaries are byte-equivalent to /tmp/new-teamvault-* → skip
```
...
|----------------|--------------|
| `keychain*.go`, `factory/`, `cmd/`, `*.go` outside test files | Full scenario walk before approving |
...
The git tag is sufficient for `go install github.com/bborbe/teamvault-utils/v4/cmd/teamvault-*@vX.Y.Z`. A GitHub Release object is separate (see next section).
...
```bash
# Operator local install (whichever binary needed)
go install github.com/bborbe/teamvault-utils/v4/cmd/teamvault-login@latest
teamvault-login --help 2>&1 | head -1  # sanity check

# Downstream / fresh machine:
go install github.com/bborbe/teamvault-utils/v4/cmd/teamvault-password@vX.Y.Z

# Library import (downstream Go projects):
#   go.mod: require github.com/bborbe/teamvault-utils/v4 vX.Y.Z
#   go.mod: github.com/bborbe/teamvault-utils/v4 vX.Y.Z
```
