...
- [ ] `START_A=$(date +%s); OUT_A=$(/tmp/new-teamvault-password --teamvault-config $WORK_DIR/config.json --teamvault-key any-key --teamvault-timeout=-1s 2>/tmp/scenario-005-A.err); RC_A=$?; END_A=$(date +%s); DURATION_A=$((END_A - START_A))`
...
- [ ] `START_B=$(date +%s); OUT_B=$(/tmp/new-teamvault-password --teamvault-config $WORK_DIR/negative-config.json --teamvault-key any-key 2>/tmp/scenario-005-B.err); RC_B=$?; END_B=$(date +%s); DURATION_B=$((END_B - START_B))`
...
```bash
rm -rf "$WORK_DIR" /tmp/new-teamvault-password /tmp/scenario-005-*.err
```
